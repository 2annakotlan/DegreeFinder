...
def append_row_by_headers(data, sheet_name="Sheet1"):
    ...
    missing_columns = [header for header in data.keys() if header not in column_headers]
    
    if missing_columns:
        column_headers.extend(missing_columns)
        column_headers.sort()  
        service.spreadsheets().values().update(spreadsheetId=spreadsheetId, range=f"{sheet_name}!1:1", valueInputOption="RAW", body={"values": [column_headers]}).execute()
