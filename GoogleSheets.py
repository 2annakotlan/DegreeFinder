...
def add_columns(sheet_name):
    ...
    sheet.values().update(spreadsheetId=spreadsheetId, range=f"{sheet_name}!1:{len(spreadsheet_degrees) + len(new_degrees)}", valueInputOption="RAW", body={"values": [new_degrees]}).execute() # adding new degrees as columns
